...
class PolygonConverter(CocoToJSONLinesConverter):
    ...
    def _populate_bbox_in_label(self, label, annotation, image_details):
        ...
        # AE-Added polygon data to output
    def _populate_polygon_in_label(self, label, annotation, image_details):   
        # ReScale from 0 to 1
        ...
        for i in range(len(segments)):
            ...
            if (i % 2) == 0:
                #even
                segments[i]=segments[i]/image_details["width"]
            else:
                # odd
                segments[i]=segments[i]/image_details["height"]

        label["polygon"]=[segments]
        # label["polygon"]=segments
    ...
    def _populate_label(self, annotation):
        ...
        #self._populate_bbox_in_label(label, annotation, image_details)     # AE-Comment this line out to supress bounding box in output json file
        self._populate_polygon_in_label(label, annotation, image_details)   # AE-Add polygon data to output
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["InstanceSegmentation"],
        default="InstanceSegmentation",
    )
    ...
    if task_type == "InstanceSegmentation":
